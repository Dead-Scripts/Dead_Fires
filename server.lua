...
RegisterNetEvent('DeadFires:AddLocationFire')
AddEventHandler('DeadFires:AddLocationFire', function(fireIndex)
	LocationFireTracker[fireIndex] = true;
end)
RegisterNetEvent('DeadFires:AddConcurrent')
AddEventHandler('DeadFires:AddConcurrent', function(firetrackID)
	local src = source;
...
RegisterNetEvent('DeadFires:StopLocationFire')
AddEventHandler('DeadFires:StopLocationFire', function(fireLocationIndex)
...
RegisterCommand("fire", function(source, args, rawCommand)
	...
	--[[
		Commands:
			...
			/fire stop <ID>
			/fire stopall - Requires `DeadFires.StopAll` permission
	...
	elseif args[1] == "stop" then 
		...
		if (not args[2]:find("L") or IsPlayerAceAllowed(src, "DeadFires.Stop.Location")) then
	...
	elseif args[1] == "preview" then 
		if #args ~= 5 then 
			TriggerClientEvent('chatMessage', src, '^1[^5DeadFires^1] ^2Preview Mode ^3has been ^1DISABLED^3...');
		...
		if Config.AnyoneCanUse then 
			...
			TriggerClientEvent('chatMessage', src, '^1[^5DeadFires^1] ^2Preview Mode ^3has been ^2ENABLED^3...');
			return;
		else 
			-- Use permission nodes 
			local perms = Config.Permissions;
